...
# build two clean plotting datasets
df_REE <- REE %>%
  mutate(
    Fitted = fitted(model_REE),
    Observed = REE,
    Model = "REE"
  ) %>%
  select(Fitted, Observed, ID, Model)

df_REEkg <- REE %>%
  mutate(
    Fitted = fitted(model_REEkg),
    Observed = REEperKG,
    Model = "REE/kg"
  ) %>%
  select(Fitted, Observed, ID, Model)
...
# combine
df_plot <- bind_rows(df_REE, df_REEkg)

# final combined plot
ggplot(df_plot, aes(x = Fitted, y = Observed, color = ID)) +
